CREATE OR REPLACE FUNCTION export_actions_prod (
    ...
    WITH id_insert_update AS (
        ...
        FROM RAW.HORIZON_PROD.HZN_REQUEST_REQUEST r
            ...
            -- Name of the Workflow Actioner
             LEFT JOIN RAW.HORIZON_PROD.HZN_REQUEST_WORKFLOW_USER wu
                ON wu.id = aa.WORKFLOW_USER_ID AND
                   wu.REQUEST_ID = aa.REQUEST_ID AND
                   wu.WORKFLOW_STEP_ID = aa.WORKFLOW_STEP_ID
             LEFT JOIN RAW.HORIZON_PROD.HZN_IDENTITY_ZUSER usr 
                ON usr.ID = wu.USER_ID
             LEFT JOIN RAW.HORIZON_PROD.HZN_MASTER_INDIVIDUAL usr_party 
                ON usr_party.PARTY_ID = usr.PARTY_ID
            -- Comment
             LEFT JOIN RAW.HORIZON_PROD.HZN_REQUEST_WORKFLOW_COMMENT rwc
