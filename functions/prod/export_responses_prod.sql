CREATE OR REPLACE FUNCTION export_responses_prod (
    ...
    AS $$
        ...
        response_details AS (
            SELECT db_insert_timestamp
             ...
             FROM RAW.HORIZON_PROD.HZN_REQUEST_RESPONSE
             WHERE tenant_id = arg_tenant_id
        ),
        ...
        attachment_details AS (
            ...
            FROM RAW.HORIZON_PROD.HZN_REQUEST_FILE_ATTACHMENT
        ...
        FROM RAW.HORIZON_PROD.HZN_REQUEST_REQUEST r
             JOIN RAW.HORIZON_PROD.HZN_REQUEST_REQUEST_ACTIVITY ra 
                ON ra.REQUEST_ID = r.ID
             JOIN RAW.HORIZON_PROD.HZN_REQUEST_REQUEST_QUESTIONNAIRE rq
                ON rq.REQUEST_ACTIVITY_ID = ra.ID AND 
                   rq.REQUEST_ID = ra.REQUEST_ID
             JOIN RAW.HORIZON_PROD.HZN_REQUEST_REQUEST_ITEM ri
                ON ri.REQUEST_QUESTIONNAIRE_ID = rq.ID
             JOIN RAW.HORIZON_PROD.HZN_REQUEST_REQUEST_VERSION rv
                  ON rv.REQUEST_QUESTIONNAIRE_ID = rq.ID
             JOIN response_details rs 
                  ON rs.REQUEST_ITEM_ID = ri.ID AND 
                     rs.REQUEST_VERSION_ID = rv.ID
             LEFT JOIN RAW.HORIZON_PROD.HZN_REQUEST_REQUEST_ACTIVITY_PARTY rap
                  ON rap.REQUEST_PARTY_ID = rq.REQUEST_PARTY_ID AND
                     rap.REQUEST_ACTIVITY_ID = rq.REQUEST_ACTIVITY_ID
             LEFT JOIN RAW.HORIZON_PROD.HZN_REQUEST_REQUEST_PARTY rrp
