CREATE OR REPLACE FUNCTION export_users_prod (
  ...
  , arg_end_date varchar
    ...
    AS $$
        WITH id_insert_update AS (
            ...
            FROM RAW.HORIZON_PROD.HZN_IDENTITY_ZUSER
        ...
        --      , i.EMPLOYEE_CATEGORY
        --      , i.SUBFUNCTION
        FROM RAW.HORIZON_PROD.HZN_IDENTITY_ZUSER z
            LEFT JOIN RAW.HORIZON_PROD.HZN_MASTER_INDIVIDUAL i 
                ON i.PARTY_ID = z.PARTY_ID
        -- Department
            LEFT JOIN RAW.HORIZON_PROD.HZN_MASTER_LIST_ITEM usr_function
                ON usr_function.CODE = i.ZFUNCTION AND 
                   usr_function.LIST_CODE = 'department' AND
                   usr_function.TENANT_ID = i.TENANT_ID
        -- Country
            LEFT JOIN RAW.HORIZON_PROD.HZN_MASTER_PARTY_COUNTRY pc
                ON pc.PARTY_ID = i.PARTY_ID
            LEFT JOIN RAW.HORIZON_PROD.HZN_MASTER_REF_COUNTRY c 
                ON c.CODE = pc.COUNTRY_CODE
            LEFT JOIN RAW.HORIZON_PROD.HZN_MASTER_PARTY p
                ON i.PARTY_ID = p.ID
