CREATE OR REPLACE FUNCTION export_activities_stage (
  ...
  , arg_end_date varchar
    ...
    AS $$
        ...
        FROM RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST r
            -- Request Activity
            JOIN RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST_ACTIVITY ra ON ra.REQUEST_ID = r.ID
            JOIN RAW.HORIZON_STAGE.HZN_REQUEST_ACTIVITY a ON a.CODE = ra.ACTIVITY_CODE AND a.TENANT_ID = r.TENANT_ID
            JOIN RAW.HORIZON_STAGE.HZN_MASTER_SYSTEM_LIST_ITEM sli ON sli.CODE = ra.ZSTATUS AND sli.LIST = 'request_status'
