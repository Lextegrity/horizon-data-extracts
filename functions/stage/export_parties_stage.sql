CREATE OR REPLACE FUNCTION export_parties_stage (
  ...
  , arg_end_date varchar
    ...
    WITH id_insert_update AS (
            ...
            FROM RAW.HORIZON_STAGE.HZN_MASTER_PARTY
            ...
            SELECT PARTY_ID                 AS id
                 ...
                 , max(DB_INSERT_TIMESTAMP) AS insert_dt
                 , max(DB_UPDATE_TIMESTAMP) AS update_dt
            FROM RAW.HORIZON_STAGE.HZN_MASTER_PARTY_INFORMATION
            WHERE tenant_id = arg_tenant_id
            GROUP BY 1, 2
            ...
            SELECT PARTY_ID                 AS id
                 ...
                 , max(DB_INSERT_TIMESTAMP) AS insert_dt
                 , max(DB_UPDATE_TIMESTAMP) AS update_dt
            FROM RAW.HORIZON_STAGE.HZN_MASTER_PARTY_FAIR_MARKET_VALUE
            WHERE TENANT_ID = arg_tenant_id
            GROUP BY 1, 2
        ),
        ...
        party_data AS (
            ...
            SELECT PARTY_ID             AS id
                 ...
                 , TENANT_ID            AS customer_id
            FROM RAW.HORIZON_STAGE.HZN_MASTER_ORGANIZATION
            WHERE TENANT_ID = arg_tenant_id
        )
        ...
        FROM party_data pd
            LEFT JOIN RAW.HORIZON_STAGE.HZN_MASTER_PARTY_FAIR_MARKET_VALUE fmv 
                ON fmv.PARTY_ID = pd.id
            LEFT JOIN RAW.HORIZON_STAGE.HZN_MASTER_PARTY party
