CREATE OR REPLACE FUNCTION export_requests_stage (
  ...
  , arg_end_date varchar
    ...
    AS $$
        ...
        created_dates AS (
            ...
            FROM RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST_VERSION
            WHERE TENANT_ID = arg_tenant_id
        ...
        version_details AS (
            SELECT rv.TENANT_ID
                 ...
                 , CASE
                   ...
                   END rownum_desc
            FROM RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST_VERSION rv
                LEFT JOIN RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST_VERSION rva 
                    ON rv.ID = rva.ID AND
                       rva.SUBMIT_TIMESTAMP is not null
                LEFT JOIN RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST_VERSION rvd
                    ON rv.ID = rvd.ID AND
        ...
        activity_sums AS (
            ...
            FROM RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST_ACTIVITY
            WHERE tenant_id = arg_tenant_id
            GROUP BY 1, 2
        )
        ...
        SELECT r.id                                 AS id
             ...
             , rh.parent_id                         AS parent_request_id
        FROM RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST r
             -- Request type
             JOIN RAW.HORIZON_STAGE.HZN_REQUEST_ACTIVITY a
        ...
        -- User
             JOIN RAW.HORIZON_STAGE.HZN_IDENTITY_ZUSER usr
                ON usr.ID = r.OWNER_USER_ID
        -- Request Hierarchy
             LEFT JOIN RAW.HORIZON_STAGE.HZN_REQUEST_REQUEST_HIERARCHY rh
                ON r.id = rh.child_id AND
                   r.tenant_id = rh.tenant_id
        -- Created Dates
